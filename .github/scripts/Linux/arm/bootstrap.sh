...
#sudo debootstrap --arch arm64 --foreign buster $BUILD_DIR
chroot $BUILD_DIR sh -c 'apt install build-essential pkg-config autoconf automake libtool'
chroot $BUILD_DIR sh -c 'portaudio19-dev libsdl2-dev libglib2.0-dev libglew-dev libcurl4-openssl-dev freeglut3-dev libssl-dev libjack-dev libavcodec-dev libswscale-dev libasound2-dev'
