...
static bool my_display_reconfigure(struct state_video_recv *s,
...
decompress_thread(void *arg)
{
        ...
        while (true) {
                ...
                if (!display_frame) { // not yet configured until probed
                        if (!decompress_display_codec_format_probed(d, &comp_desc)) {
                                continue;
                        }
...
static bool
my_display_reconfigure(struct state_video_recv *s, struct video_desc desc)
{
...
{
        struct state_video_recv *s = d->s;
        unsigned tile_width = recv_frame->tiles[0].width; // get_video_mode_tiles_x(decoder->video_mode);
        unsigned tile_height = recv_frame->tiles[0].height; // get_video_mode_tiles_y(decoder->video_mode);
        ...
        for (unsigned i = 0; i < recv_frame->tile_count; ++i) {
                ...
                if (!display_frame) {
                        ...
                        if (d->merged_fb) {
                                ...
                                buf = (unsigned char *) display_frame->tiles[i].data;
                        }
